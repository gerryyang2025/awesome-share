...
| **`assets/css/jekyll-theme-chirpy.scss`** | **Official** custom-style entry (same pattern as [chirpy-starter](https://github.com/cotes2020/chirpy-starter)): `@use 'main'` / `main.bundle` (production), then **`@use 'toc-expand' as *`** (TOC always fully expanded), then **`@use 'post-content-bundle' as *`** so site CSS is compiled **with** the theme into **`/assets/css/jekyll-theme-chirpy.css`**. |
| **`_sass/toc-expand.scss`** | **Contents / TOC**: overrides Chirpy’s progressive-hide styling so nested lists under **`#toc`** (right panel) and **`#toc-popup-content`** (mobile modal) stay **`display: block`**. Pairs with **`collapseDepth:0`** in vendored **`assets/js/dist/post.min.js`**. |
| **`_sass/_post-content-bundle.scss`** | **Generated** — do not hand-edit; concatenation order: **markdown-extras → blockquote → table → chirpy-code-blocks → inline-code → rouge-syntax-tokens**. Regenerate with **`./tools/bundle-post-content-css.sh`** or **`./optools bundle-content-css`**. |
| **`_includes/metadata-hook.html`** | Theme **head** hook (optional `<link>` / meta). Post styles are **not** loaded here; they follow the row above. |
...
| **`assets/css/chirpy-code-blocks.css`** | Fallback **`--highlight-bg-color`** / border on **`div[class^='language-']`** and inner **`.highlight`** so fenced Rouge blocks match Chirpy’s code “card” when theme vars or order differ. |
| **`assets/css/inline-code.css`** | Aligns with Chirpy **`_sass/base/_syntax.scss`**: **no global `color` on `.content code`** (avoids masking Rouge spans); **pill** on **`code.highlighter-rouge`**; prose **`p`/`li`/heading `code`** get **`--code-color`**; block **`.highlight > code`** has no forced text color. |
| **`assets/css/rouge-syntax-tokens.css`** | Scoped **`.content .highlight code .k`** (etc.) light/dark palette ≈ **Rouge GitHub** theme — backup if Chirpy’s compiled token rules are missing or lose to `@use` order. |

**Changing the look later:** edit the source files under `assets/css/` (same order as the bundle script), then run **`./tools/bundle-post-content-css.sh`** so **`_sass/_post-content-bundle.scss`** updates. Prefer adjusting **`--md-*`** at the top of `markdown-extras.css` first. Shell colors (sidebar, navbar) and code-block palettes live in the theme’s SCSS unless you add more `@use` partials next to **`jekyll-theme-chirpy.scss`**. TOC chrome is **`_sass/toc-expand.scss`** (not part of the generated bundle).
...
- **Contents (TOC)**:
  - With `toc: true` in `_config.yml` (posts default to this in `defaults`), Chirpy shows **Contents** on the **right** from the **`xl`** breakpoint upward, and a **mobile TOC** control below that—see [Writing a New Post · Chirpy](https://chirpy.cotes.page/posts/write-a-new-post/).
  - **Default full expand:** nested headings are listed in full (no progressive “reveal as you scroll”). Implemented by **`_sass/toc-expand.scss`** (imported from **`assets/css/jekyll-theme-chirpy.scss`**) and by **`collapseDepth:0`** on **both** tocbot instances in vendored **`assets/js/dist/post.min.js`** (desktop **`#toc`** and popup **`#toc-popup-content`**). To revert to stock Chirpy behavior, remove the **`@use 'toc-expand'`** line and restore the gem’s **`post.min.js`** (or drop **`collapseDepth`** / set it back to the upstream value after diffing).
  - **Vendored `assets/js/dist/post.min.js`** (diff vs the theme gem after each **`jekyll-theme-chirpy`** upgrade): **(1)** **`headingSelector`** includes **`h1`–`h4`** inside `.content` (stock tocbot only **`h2`–`h4`**, so a lone body **`#`** never appeared; the post title `<h1 data-toc-skip>` stays outside `.content` and is still excluded). **(2)** **`collapseDepth:0`** for both TOC panels (pairs with **`toc-expand.scss`**). **(3)** ClipboardJS **`target`** falls back to **`td.rouge-code`** / **`.highlight code`** when **`code .rouge-code`** is missing (Rouge line-table vs no-line-number markup). Chirpy does not ship a hook for these; re-copy **`post.min.js`** from the new gem and re-apply the edits.
  - In-article **`{:toc}`** (`#markdown-toc`) stays **hidden** when the theme TOC is on (`article[data-toc="true"]`). Use **`toc: false`** on a post if you only want kramdown **`{:toc}`** (wide screens: sticky column styling in **`assets/css/markdown-extras.css`**).
