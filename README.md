...
### Search (top-right panel, [SimpleJekyllSearch](https://github.com/christian-fei/Simple-Jekyll-Search))

Chirpy’s UI matches the [official demo](https://chirpy.cotes.page/): the panel loads **`/assets/js/data/search.json`**, built from **`search.json.html`**. The theme’s loader defaults to **`limit: 10`** results; with many posts, the engine walks **`site.posts` (newest first)** and **stops after 10 matches**. Queries like **“CPP Lab”** are split on spaces (`cpp` + `lab`); **every token must appear in the same JSON field** (title, url, categories, tags, or content). Newer technical posts often satisfy that on **`content`** (e.g. `cpp` in code plus `lab` inside words like *label*), so the quota fills before older articles whose **title** is “CPP Lab” are ever scanned. This repo overrides **`_includes/search-loader.html`** with a higher **`limit`** (500) so the full index can match. **`search.json.html`** is aligned with Chirpy: **`url`** via `relative_url`, **`categories` / `tags`** joined with `', '`, a **`date`** field, and **`content`** from **`_includes/post-summary.html`** (`full_text`) so indexing matches the theme. After upgrading Chirpy, diff **`search-loader.html`** and **`post-summary.html`** against the gem. Theme `search.json` uses `layout: compress`; this site keeps **`layout: null`** so the output stays valid JSON under `compress_html`.
...
4. **Search** – Chirpy already ships [SimpleJekyllSearch](https://github.com/christian-fei/Simple-Jekyll-Search) + `search.json`; this repo overrides `_includes/search-loader.html` and `search.json.html` (see **Search** under Theme: Chirpy). Optional: raise `limit` further or add fuzzy mode if needed.
