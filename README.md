...
| **`assets/css/jekyll-theme-chirpy.scss`** | **Official** custom-style entry (same pattern as [chirpy-starter](https://github.com/cotes2020/chirpy-starter)): `@use 'main'` / `main.bundle` (production), then **`@use 'post-content-bundle' as *`** so site CSS is compiled **with** the theme into **`/assets/css/jekyll-theme-chirpy.css`**. |
...
- **Contents (TOC)**: With `toc: true` in `_config.yml` (posts default to this in `defaults`), Chirpy shows the **Contents** panel on the **right** on large viewports and the mobile TOC control below that breakpoint—same idea as [Writing a New Post · Chirpy](https://chirpy.cotes.page/posts/write-a-new-post/). The stock theme’s [tocbot](https://github.com/tscanlin/tocbot) only scans **`h2`–`h4`** inside `.content`, so a body `#` heading never appeared in the panel; this repo **vendors** `assets/js/dist/post.min.js` with two small fixes vs the gem: **(1)** **`headingSelector`** includes **`h1`–`h4`** (the post `<h1 data-toc-skip>` is outside `.content`, so it stays excluded). **(2)** ClipboardJS **`target`** falls back to **`td.rouge-code`** / **`.highlight code`** when **`code .rouge-code`** is missing (Rouge line-table vs no-line-number markup). After upgrading **`jekyll-theme-chirpy`**, re-copy **`post.min.js`** from the new gem and re-apply those edits (Chirpy does not ship a script hook for either). In-article `{:toc}` stays hidden when the theme TOC is on; use `toc: false` if you only want kramdown `{:toc}` (wide screens: right column in `assets/css/markdown-extras.css`).
