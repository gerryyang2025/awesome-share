...
| **`_sass/_post-content-bundle.scss`** | **Generated** — do not hand-edit; concatenation order: **markdown-extras → blockquote → table → chirpy-code-blocks → inline-code**. Regenerate with **`./tools/bundle-post-content-css.sh`**. |
...
| **`assets/css/inline-code.css`** | Aligns with Chirpy **`_sass/base/_syntax.scss`**: base **`.content code`** (color + font); **pill** only on **`.content code.highlighter-rouge`**; block reset on **`.content .highlight > code`** (no `color: inherit`, so Rouge token spans keep theme colors). |
