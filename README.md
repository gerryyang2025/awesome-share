...
- **Contents (TOC)**: With `toc: true` in `_config.yml` (posts default to this in `defaults`), Chirpy shows the **Contents** panel on the **right** on large viewports and the mobile TOC control below that breakpoint—same idea as [Writing a New Post · Chirpy](https://chirpy.cotes.page/posts/write-a-new-post/). In-article `{:toc}` is hidden when the theme TOC is active so you do not see two lists. Set `toc: false` in a post’s front matter if you only want the kramdown `{:toc}` block (on wide screens it is laid out in a right column via `assets/css/markdown-extras.css`).
