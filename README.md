...
| **`assets/css/inline-code.css`** | Inline `` `code` `` matches Chirpy **`_syntax.scss`**: **`0.85rem`**, **`3px 5px`** padding, **`6px`** radius, **`var(--code-color)`** / **`var(--inline-code-bg)`** (see theme `_sass/themes/_light.scss` / `_dark.scss`). **`article .content`** included for Chirpy’s post body. Blockquote inline code uses **`color: inherit`** like upstream. |
...
4. **Search** – Implemented with **`assets/js/blog-search.js`** (debounced, ranked); see **Search** under Theme: Chirpy. Optional: tune `debounceMs`, `maxResults`, or `truncate` in `search.json.html`.
