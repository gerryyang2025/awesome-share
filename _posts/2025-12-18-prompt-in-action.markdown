...
* 参考原始的 A 游戏，在当前工程 B 目录复刻实现支持微信小游戏的版本。不确定的问题通过查询微信小游戏官方文档来确认。首先在 B 目录下完成游戏设计文档，分析记录具体的修改步骤
* 根据 B 目录下的游戏设计文档，开始分步骤修改
* 美术风格表现的要求，需要保证整套风格会真正统一。虽然存在微信小游戏上传的 4MB 主包限制，但是仍需要迁移必要的美术资源，然后再通过压缩解决尺寸大的问题，不要因为尺寸不符合要求而放弃，实在没有办法的场景再选择手绘美术
* 游戏所有 UI 界面的布局需要合理，控件之间的距离需要适当，控件的大小需要适配页面布局，控件与控件之间，控件与文字之间不能够存在遮挡重叠，所有 UI 界面的风格需要一致
