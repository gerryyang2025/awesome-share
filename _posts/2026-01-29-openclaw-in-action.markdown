...
1. OpenClaw 有一双能干活的手，但没有脑子。可参考软件开发的设计方法，在架构上拆分多 Agent，让不同任务各用各的脑，通过合理的方式来让它更聪明。
2. 成本顾虑，使用 Coding Plan 后付费模式，基本 Token 用不完 (PS: 多模态模型服务需要额外收费)。
3. 沙盒模式使用 OpenClaw，可规避大部分安全问题，试错成本更低。
4. 如何去除 AI 味，增加更多人文的温度，这个可能是碳基生物最大的优势。
5. 让子弹飞一会儿，找到合适的场景最重要，不要盲目拿着锤子找钉子。
