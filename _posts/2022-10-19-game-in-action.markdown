...
* Text-to-Speech AI (`TTS`)
  + 可以使用原生文字转语音 (TTS) 生成功能将文本输入转换为单人或多人音频。文字转语音 (TTS) 生成是可控的，这意味着您可以使用自然语言来构建互动，并指导音频的风格、口音、节奏和语气。

* 指挥官模式
  + 指挥模式指的是 NPC 不局限于**对话**，还能进行一些**操作**。
...
`SLG` 在游戏领域主要指 **Simulation Game** (模拟游戏)，尤指其中的 **策略游戏** (Strategy Game)，**核心玩法常围绕“探索、扩张、开发、征服”**，涵盖**战争模拟**、**经营模拟**等，如《三国志》系列、《率土之滨》；也可指 **Simulation Role-playing Game (策略角色扮演)**，如《火焰纹章》；而在体育（如棒球长打率）或物流公司等领域也有其他特定含义。

> 1. 90% 的成本都用在买量上。
> 2. 团队大概在 30 人左右，非常轻量，很多工作还可以借助 AI 来完成。
