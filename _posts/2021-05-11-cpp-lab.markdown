...
## [std::chrono::duration](https://cplusplus.com/reference/chrono/duration/)
...
Internally, the object stores the **count** as an object of member type `rep` (an alias of the first template parameter, `Rep`), which can be retrieved by calling member function [count](https://cplusplus.com/duration::count).

This [count](https://cplusplus.com/duration::count) is expresed in terms of **periods**. The length of a **period** is integrated in the type (on compile time) by its second template parameter (Period), which is a [ratio](https://cplusplus.com/ratio) type that expresses the number (or fraction) of seconds that elapse in each **period**.

> Template parameters

* Rep

An arithmetic type, or a class emulating an arithmetic type, to be used as the type for the internal [count](https://cplusplus.com/duration::count).

* Period
*
A [ratio](https://cplusplus.com/ratio) type that represents the **period** in seconds.
...
//hours_type h_onehour (s_onehour);          // NOT VALID (type truncates), use:
  ...
  std::cout << ms_onehour.count() << "ms in 1h" << std::endl; // Output: 3600000ms in 1h
...
## [std::error_code](https://en.cppreference.com/w/cpp/error/error_code.html)

`std::error_code` represents a platform-dependent error code value. Each std::error_code object holds an error code value originating from the operating system or some low-level interface and a pointer to an object of type [std::error_category](https://en.cppreference.com/w/cpp/error/error_category.html), which corresponds to the said interface. The error code values are not required to be unique across different error categories.
